...
exports.welcome_teacher_email = function(email, activation_route, name, username) {
    EmailMessage.findOne( { idName: 'instructor-activacion' } ).exec(function (err, result) {
        ...
        let mailOptions = {
            ...
            bcc: constants.hidden_trace_mail,
...
exports.notificate_new_lesson = function (lessons) {
    async.parallel({
        ...
        let lessonsPeriod;
        if (lessons.length > 1) {
            lessonsPeriod = "Fechas entre: " + lessons[0].getNiceDate(lessons[0].date, 540 * 60 * 1000) + " y " +
                lessons[lessons.length-1].getNiceDate(lessons[lessons.length-1].date, 540 * 60 * 1000);
        } else {
            lessonsPeriod = "Fecha: " + lessons[0].getNiceDate(lessons[0].date, 540 * 60 * 1000);
        }
        let data = results.email.body.replace('#TEACHERNAME#', results.teacher.firstName)
            ...
            .replace('#ADDRESS#', lessons[0].address);
        let mailOptions = {
            from: '"'+constants.smtp_from_name+'" <'+constants.smtp_from_email+'>', // sender address
...
exports.new_message_received = function(conversation) {
    // setup e-mail data with unicode symbols
    let sender, receiver;
    if (conversation.message[conversation.message.length - 1].sender.equals(conversation.user1.user.id)) {
        sender = conversation.user1.user;
        receiver = conversation.user2.user;
    } else {
        sender = conversation.user2.user;
        receiver = conversation.user1.user;
    }
    EmailMessage.findOne( { idName: 'mensaje-recibido-chat' } ).exec(function (err, result) {
        result.body = result.body.replace('#RECEIVERNAME#', receiver.owner).replace('#SENDERNAME#', sender.owner)
            .replace('#MESSAGE#', conversation.message[conversation.message.length - 1].text);
        let mailOptions = {
            from: '"'+constants.smtp_from_name+'" <'+constants.smtp_from_email+'>', // sender address
            to: receiver.email, // list of receivers
            bcc: constants.hidden_trace_mail,
            subject: 'Nuevo mensaje recibido en Instructorio', // Subject line
            html: result.body,
            text: sender.owner + ' te ha enviado un mensaje.'
        };
        // send mail with defined transport object
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
...
exports.landing_message = function(info) {
    // setup e-mail data with unicode symbols
    EmailMessage.findOne( { idName: 'mensajes-landing' } ).exec(function (err, result) {
        result.body = result.body.replace('#NAME#', info.name).replace('#EMAIL#', info.email)
            .replace('#INTEREST#', info.interest).replace('#OPTIONS#', info.options)
            .replace('#EMAIL_TYPE#', info.emailType);
        let mailOptions = {
            from: '"'+constants.smtp_from_name+'" <'+constants.smtp_from_email+'>', // sender address
            to: constants.hidden_trace_mail, // list of receivers
            //bcc: constants.hidden_trace_mail,
            subject: 'Informacion desde Landing', // Subject line
            html: result.body,
            text: 'Alguien ha dejado datos'
        };
        // send mail with defined transport object
        transporter.sendMail(mailOptions, function (error, info) {
            if (error) {
